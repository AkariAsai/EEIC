...
double solve(const int n, const int all_cities, int route[], double sum_d,
             int curr) {
  int i;

  if (n == 1) {
    sum_d += distance(curr, 0);
    if (sum_d < min_d) {
      min_d = sum_d;
  ...
  //  for (i = all_cities - n; i < all_cities; i++) {
  for (int j = 1; j < all_cities; j++) {
    if (!visited[j]) {
      route[all_cities - n] = j;
      visited[j] = 1;
      double temp = distance(curr, j);
      double dis = solve(n - 1, all_cities, route, sum_d + temp, j);
      visited[j] = 0;
    }
  }
  //}
...
int main(const int argc, const char **argv) {
  ...
  const double d = solve(n, n, route, 0, 0);
