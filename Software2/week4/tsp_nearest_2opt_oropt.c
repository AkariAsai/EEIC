// This is combinatio of nearest neighbor, opt2 and oropt. Becomes better as the
// number of cities increases than NN-OPT2.
/*
   n = 3 total distance = 89.790041
   n = 5 total distance =  110.417246
   n = 8 total distance =  138.864024
   n = 10 total distance =  148.256097
   n = 20 total distance = 188.658713
   n = 30 total distance =  239.668059
 */
...
// This is OROPT method.
void improve_route_or_opt(int n) {
  int temp[MAX_CITIES] = {0};
  for (int i = 0; i < n; i++) {
    int i_prev = i - 1;
    int i_next = i + 1;
    if (i_prev < 0) {
    ...
    for (int j = 0; j < n; j++) {
      ...
      if (j != i && j_next != i) {
        double l1 = distance(result[i_prev], result[i]);
        double l2 = distance(result[i], result[i_next]);
        double l3 = distance(result[j], result[j_next]);
        double l4 = distance(result[i_prev], result[i_next]);
        double l5 = distance(result[j], result[i]);
        double l6 = distance(result[i], result[j_next]);
